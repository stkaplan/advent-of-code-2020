...
def should_activate(cells, x, y, z):
    current = (x, y, z) in cells
    count = 0
    for xx in range(x-1, x+2):
        for yy in range(y-1, y+2):
            for zz in range(z-1, z+2):
                if (xx, yy, zz) == (x, y, z): # Skip current cell.
                    continue
                if (xx, yy, zz) in cells:
                    count += 1
    ...
    for x in range(min_x-1, max_x+2):
        for y in range(min_y-1, max_y+2):
            for z in range(min_z-1, max_z+2):
                if should_activate(cells, x, y, z):
                    new_cells.add((x, y, z))
...
class Test(unittest.TestCase):
    def test_iterate(self):
        with open('test1.txt') as f:
            cells = parse_input(f)
        self.assertEqual(cells, set([(1, 0, 0), (2, 1, 0), (0, 2, 0), (1, 2, 0), (2, 2, 0)]))
